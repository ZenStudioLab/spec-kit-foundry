...
- If absent: use defaults (`granularity: task`, default commit message template).
- If present: parse YAML and apply `granularity` and `commit_message_template` values.
...
1. `git status --porcelain` → skip if empty (nothing to commit)
2. `git add -A`
3. `git commit -m "feat(<featureId>): <taskText>"` (or configured template)
4. If non-zero exit → **halt**, report error, wait for user
