...
A developer working on a feature wants an independent critical review before moving to the next phase. They invoke `/speckit.peer.review <target>` naming either one of the built-in spec artifacts (`spec`, `research`, `plan`, `tasks`) or an explicit file path. Artifact targets use the peer pack review loop and append rounds to the feature review history. Explicit file paths delegate to `/plan-review` for that file instead of entering the peer pack artifact workflow. If an artifact needs revision, the developer addresses the feedback and re-runs the command; each iteration adds a new round. The loop ends when the review resolves to `MOSTLY_GOOD` or `APPROVED`.
...
6. **Given** the user runs `/speckit.peer.review docs/plans/example.md`, **Then** the command delegates to `/plan-review docs/plans/example.md` and does not create or update any `specs/<feature>/reviews/*` peer-review artifact.
...
- Concurrent artifact review invocations against the same review file are unsupported in v1; users should run review rounds sequentially.

## Requirements *(mandatory)*

### Functional Requirements

- **FR-001**: Users MUST be able to invoke `/speckit.peer.review` with either one of four artifact targets (`spec`, `research`, `plan`, `tasks`) or an explicit existing file path.
- **FR-002**: Each artifact-target review invocation MUST produce or update a review file at `specs/<feature>/reviews/<artifact>-review.md`.
- **FR-003**: For sequential invocations, review rounds MUST be append-only; rounds are separated by `---` and new rounds are appended at the end of the file; no previous round may be overwritten or deleted. Concurrent artifact review invocations against the same review file are unsupported in v1.
...
- **FR-011**: In artifact mode, provider selection MUST be configurable via `default_provider` in `.specify/peer.yml` and overridable per-invocation via `--provider <name>`. File-path delegation mode bypasses peer provider selection.
...
- **FR-013**: In artifact mode, provider session state MUST be persisted in `specs/<feature>/reviews/provider-state.json` keyed by provider and workflow type to enable session reuse across rounds. File-path delegation mode MUST NOT create or update provider state.
...
- **Review Target**: Either a built-in artifact name handled by the peer pack review loop, or an explicit file path delegated to `/plan-review`.
...
- **SC-002**: 100% of review rounds are preserved across sequential invocations — no prior round is overwritten.
- **SC-003**: All six core test scenarios are covered after a fresh `--dev` install of the `peer` pack: (1) review for each of the four artifact types produces an output file, (2) explicit file-path review delegates to `/plan-review` without creating or updating peer review state, (3) execute with session reuse skips completed tasks, (4) default provider is used when no flag is given, (5) `--provider codex` override works, (6) an unimplemented provider request fails with a clear error. Coverage may be split between the automated validation matrix and explicit manual delegation checks.
...
- File-path delegation mode depends on a host-provided `/plan-review` skill or command; the `peer` pack does not bundle or install it.
- Artifact review rounds are expected to run sequentially per review file in v1; concurrent artifact review invocations against the same target are unsupported.
...
- The Codex adapter invokes the codex skill script at `~/.claude/skills/codex/scripts/ask_codex.sh`. Calls may emit `session_id` and `output_path` metadata on stdout; when present, `session_id` is stored in `provider-state.json` and reused via `--session <id>` on subsequent rounds. The provider-written review file remains the source of truth for artifact review outcomes.
